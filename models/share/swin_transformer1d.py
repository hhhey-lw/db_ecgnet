...
def window_partition(x, window_size):
    """
    Returns:
        windows: (batch_size * num_windows, window_size, C)
    """
    B, L, C = x.shape
...
def window_reverse(x, window_size, L):
    """
    Returns:
        x: (B, L, C)
    """
    B = int(x.shape[0] / (L / window_size))
    x = x.view(B, L // window_size, window_size, -1)
    x = x.reshape(B, L, -1)
    return x
...
class SwinTransformerBlock1D(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        _, Lp, _ = x.shape
        # cyclic shift
        if shift_size > 0 and L != self.window_size:
            shifted_x = torch.roll(x, shifts=-shift_size, dims=(1))  # 往左移动
...
class WindowAttention1D(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # make torch-script happy (cannot use tensor as tuple)
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)]\
            .view(self.window_size, self.window_size, -1)  # Wl,Wl,nH
...
class PatchEmbed1D(nn.Module):
    ...
    def __init__(self, patch_size=10, in_chans=12, embed_dim=120, norm_layer=None):
        super().__init__()
...
class SwinTransformer1DStage(nn.Module):
...
"""
    The code comes from: https://github.com/meraks/Swin-Transformer-1D
    paper: Swin Transformer Hierarchical Vision Transformer using Shifted Windows
         ⭐ window attention => shift-window attention. [information interaction in window intra and window inter]
"""


def swin_transformer(num_classes=9, input_channels=12):
    return SwinTransformer1D(num_classes=num_classes, in_chans=input_channels, embed_dim=240, patch_size=20, window_size=25)
...
if __name__ == "__main__":
    ...
    # FLOPs: 355.02M
    # Params: 12.77M
    # ⭐Gradient problem, need to adjust learning rate => 0.0001, performance is similar to ViT
