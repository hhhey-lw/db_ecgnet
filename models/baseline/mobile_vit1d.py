...
class MobileViTBlock(nn.Module):
    ...
    def unfolding(self, x: Tensor) -> Tuple[Tensor, Dict]:
        ...
        batch_size, in_channels, orig_w = x.shape
        ...
        if new_w != orig_w:
            ...
            x = F.interpolate(x, size=new_w, mode="linear", align_corners=False)
            interpolate = True

        # number of patches along width and height
        num_patch_w = new_w // patch_w  # n_w

        x = x.reshape(batch_size, in_channels, num_patch_w, patch_w)
        # [B, C, N, P] -> [B, P, N, C]
        x = x.transpose(1, 3)
        # [B, P, N, C] -> [BP, N, C]
        x = x.reshape(batch_size * patch_w, num_patch_w, -1)

        info_dict = {
            "orig_size": orig_w,
            "batch_size": batch_size,
            "interpolate": interpolate,
            "total_patches": num_patch_w
        }
    ...
    def folding(self, x: Tensor, info_dict: Dict) -> Tensor:
        ...
        # [BP, N, C] --> [B, P, N, C]
        x = x.contiguous().view(
            info_dict["batch_size"], self.patch_w, info_dict["total_patches"], -1
        )

        batch_size, pixels, num_patches, channels = x.size()
        # num_patch_h = info_dict["num_patches_h"]
        num_patch_w = info_dict["total_patches"]

        # [B, P, N, C] -> [B, C, N, P]
        x = x.transpose(1, 3)
        # [B, C, N, P] -> [B*C*n_h, n_w, p_h, p_w]
        x = x.reshape(batch_size, channels, num_patch_w * self.patch_w)
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 9, input_channels=12):
        super().__init__()

        # image_channels = 3
        ecg_leads = input_channels
        out_channels = 16
        ...
        self.classifier.add_module(name="global_pool", module=nn.AdaptiveAvgPool1d(1))
...
def mobile_vit(num_classes: int = 9, input_channels=12):
    # pretrain weight link
    # https://docs-assets.developer.apple.com/ml-research/models/cvnets/classification/mobilevit_s.pt
    config = get_config("small")
    m = MobileViT(config, num_classes=num_classes, input_channels=input_channels)
    return m


"""
    code : https://github.com/apple/ml-cvnets/blob/main/cvnets/models/classification/mobilevit.py
    paper: MOBILEVIT: LIGHT-WEIGHT, GENERAL-PURPOSE, AND MOBILE-FRIENDLY VISION TRANSFORMER
    note : 1. make only the necessary changes. self attention [batch, channel, H, W] => [batch, channel, length].
           2. attention: Grid Attention, token execution attention at the same location in different Windows.
           3. avoiding gradient problem [Activation function]: 1. SiLU => GELU
"""


if __name__ == '__main__':
    input_size = (1, 12, 1000)
    net = mobile_vit(num_classes=9)

    input_data = torch.randn(size=input_size)
    out = net(input_data)
    print(out.shape)
    ...
    flops, params = profile(net, inputs=(torch.randn(*(1, 12, 1000)),))
    # print(f"FLOPs : {clever_format(flops)}")
    print(f"Params: {clever_format(params)}")

    flops = profile_macs(net, torch.randn((1, 12, 1000)))
    print(f"FLOPs : {clever_format(flops)}")
