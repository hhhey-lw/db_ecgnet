...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):
        ...
        n, c, h, w = x.size()
        # b, c, L => b, c, 1, L => AdaptiveAvgPool2d 相当于 按行
        x_h = self.pool_h(x)
...
"""
    ...
    paper: A Multi-View Multi-Scale Neural Network for Multi-Label ECG Label ECG Classification
"""


if __name__ == '__main__':
    ecg_data = torch.rand(size=(1, 12, 1000))
    net = MyNet6View(num_classes=9)
    # out = net(ecg_data)
    # print(out.shape)
    ...
    _, params = profile(net, inputs=(torch.randn(*(1, 12, 1000)),))
