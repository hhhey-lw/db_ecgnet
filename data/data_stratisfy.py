...
# ==> code url: https://github.com/helme/ecg_ptbxl_benchmarking <===

def stratisfy_df(df, new_col_name, n_folds=10, nr_clean_folds=0):
    # compute qualities as described
    quals = []
    ...
    stratified_data_ids, stratified_data = stratify(plabels, classes, [1 / n_folds] * n_folds, pquals, ecgs_per_patient,
                                                    nr_clean_folds)
...
def stratify(data, classes, ratios, qualities, ecgs_per_patient, nr_clean_folds=1):
    ...
    # data is now always a list of lists; len(data) is the number of patients; data[i] is the list of all labels
    # for patient i (possibly multiple identical entries)
